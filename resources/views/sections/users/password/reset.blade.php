...
<x-layouts::users-password>
    <div class="card card-md">
        <div class="card-body">
            ...
            <x-ui.form.index action="{{ route('password.store') }}">
                {{-- Password Reset Token --}}
                <input type="hidden" name="token" value="{{ request()->route('token') }}" autocomplete="off">
                {{-- Email Address --}}
                <div class="mb-3">
                    <label for="email" class="d-none">Эл. почта</label>
                    <input id="email" type="email" name="email" value="{{ old('email', request()->input('email')) }}" class="form-control" placeholder="Эл. почта" required>
